...
class AnnuaireUtilisateur:
    ...
    def authentifier(self):
        ...
        Si l'adresse email entrée par l'utilisateur est valide et qu'elle correspond bel et bien à l'adresse email de l'un des utilisateurs présents au niveau de la base de données,
        la méthode devra ensuite demander à l'utilisateur d'entrer son mot de passe. Si le mot de passe entré par l'utilisateur n'est pas le bon, il faudra afficher le message "Mot de passe incorrect." 
        et redemander à l'utilisateur son mot de passe. Si le mot de passe entré par l'utilisateur s'avère par contre être le bon, la méthode devra retrouver l'utilisateur correspondant à la combinaison email/mot de passe entrée.
        ...
        email_saisi = None
        while email_saisi is None:
            email_saisi = input("Veuillez entrer l'adresse email associée à votre compte: ").lower()

            if not est_une_adresse_email_valide(email_saisi):
                print("L'adresse email entrée est invalide.")
                email_saisi = None
        
            elif email_saisi not in adresses_emails_existantes:
                print("Nous n'avons trouvé aucun utilisateur avec cette adresse email au niveau de notre système.")
                email_saisi = None
            
        mot_de_passe_en_clair = None
        while mot_de_passe_en_clair is None:
            mot_de_passe_en_clair = input("Veuillez entrer votre mot de passe: ")

            for user in self.utilisateurs:
                if user.email == email_saisi:
                    if HacheurDeMotDePasse.verifier(user.mot_de_passe, mot_de_passe_en_clair):
                        return user
                    else:
